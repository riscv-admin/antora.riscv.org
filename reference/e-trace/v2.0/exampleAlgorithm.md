# 9.1. Reference Compressed Branch Trace Algorithm
...
* _resync count._ A counter used to keep track of when it is necessary to send a synchronization packet (see [9.1.2\. Resynchronisation](#sec:resync));
* _resync2\_br._ The resync FSM is in state 2 and there are entries in the branch map that have not yet been output (see[9.1.2\. Resynchronisation](#sec:resync)).
* _resync3._ The resync FSM is in state 3 (see [9.1.2\. Resynchronisation](#sec:resync));
...
### [](#9-1-1-format-selection)9.1.1\. Format selection
...
### [](#sec:resync)9.1.2\. Resynchronisation
...
### [](#rec:multiretcon)9.1.3\. Multiple retirement considerations
