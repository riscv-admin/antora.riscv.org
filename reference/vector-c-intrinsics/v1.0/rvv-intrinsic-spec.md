...
## [](#header-file-inclusion)Header file inclusion
...
## [](#control-of-vector-programming-model)Control of the vector extension programming model
...
In this chapter, we cover how the intrinsics embed the control of `vtype` fields in the function names. Please see [_Naming scheme_](#naming-scheme) for the rules described in this chapter.

### [](#control-of-effective-element-width-eew-and-effective-lmul-emul)Control of effective element width (EEW) and effective LMUL (EMUL)
...
To see the full list of data types for the intrinsics, please see [_Type system_](#type-system).
...
### [](#control-of-vl)Control of number of elements to be processed
...
The intrinsics fuse the control of vector masking (`vm`) together with the control for policy behavior (`vta`, `vma`) in the same suffix. Please checkout [Control of vector masking](#control-of-masked) and [Policy and masked naming scheme](#policy-and-masked-naming-scheme) for the exact suffix that specifies a masked/unmasked vector operation along with its policy behavior.

### [](#control-of-fixed-point-rounding-mode)Control of fixed-point rounding mode
...
### [](#control-of-frm)Control of floating-point rounding mode
...
#### [](#implicit-fp-rounding-mode-intrinsics)Implicit FP rounding mode intrinsics
...
#### [](#explicit-frm)Explicit FP rounding mode intrinsics
...
As mentioned in [Control of vector masking](#control-of-masked) and [Control of behavior of destination tail elements and destination inactive masked-off elements](#control-of-policy), the intrinsics fuses the control of `vm`, `vta`, and `vma` into the same suffix. [Policy and masked naming scheme](#policy-and-masked-naming-scheme) enumerates the exact suffixes. You may find where these suffixes are appended in [Explicit (Non-overloaded) naming scheme](#explicit-naming-scheme).
...
The explicit (non-overloaded) intrinsics embed the control described in [_Control of the vector extension programming model_](#control-of-vector-programming-model) in the function name. This scheme gives intrinsic codebase more readability as the execution states are explicitly specified in the code.
...
This section covers the general naming rule of the two types of intrinsics accordingly. Then, this section also enumerates the exceptions and the rationales behind them in [Exceptions in the explicit (non-overloaded) naming scheme](#explicit-exception-naming) and [Exceptions in the implicit (overloaded) naming scheme](#implicit-exception-naming).

### [](#policy-and-masked-naming-scheme)Policy and masked naming scheme

With the default policy scheme mentioned under [Control of behavior of destination tail elements and destination inactive masked-off elements](#control-of-policy), each intrinsic provides corresponding variants for their available control of `vm`, `vta` and `vma`. The following list enumerates the possible suffixes.
...
| |  Pseudo intrinsics mentioned under [_Pseudo intrinsics_](#pseudo-intrinsics) do not map to real vector instructions. Therefore these intrinsics are not affected by the policy setting, nor do they have intrinsic variants of the suffixes listed above. |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

### [](#explicit-naming-scheme)Explicit (Non-overloaded) naming scheme

In general, the intrinsics are encoded as the following. The intrinsics under this naming scheme are the "non-overloaded intrinsics", which in parallel we have the "overloaded intrinsics" defined under [Implicit (Overloaded) naming scheme](#implicit-naming-scheme).
...
* `RETURN_TYPE` depends on whether the return type of the vector instruction is a mask register…​  
   * For intrinsics that represents instructions with a non-mask destination register:  
         ...
         * [_Type system_](#type-system) explains the limited enumeration of EEW-EMUL pairs.  
...
* `ROUND_MODE` is the `_rm` suffix mentioned in [Explicit FP rounding mode intrinsics](#explicit-frm). Other intrinsics do not have this suffix.
* `POLICY` are enumerated under [Policy and masked naming scheme](#policy-and-masked-naming-scheme).

The general naming scheme is not sufficient to express all intrinsics. The exceptions are enumerated in the proceeding section [Exceptions in the explicit (non-overloaded) naming scheme](#explicit-exception-naming).

### [](#explicit-exception-naming)Exceptions in the explicit (non-overloaded) naming scheme

This section enumerates the exceptions in the explicit (non-overloaded) naming scheme.

#### [](#scalar-move-instructions)Scalar move instructions
...
#### [](#add-with-carry-subtract-with-borrow-instructions)Add-with-carry / Subtract-with-borrow instructions
...
#### [](#vreinterpret-vlmul%5Ftruncvlmul%5Fext-and-vsetvget)`vreinterpret`, `vlmul_trunc`/`vlmul_ext`, and `vset`/`vget`
...
### [](#implicit-naming-scheme)Implicit (Overloaded) naming scheme

The implicit (overloaded) interface aims to provide a generic interface that takes values of different EEW and EMUL as the input. Therefore, the implicit intrinsics omit the EEW and EMUL encoded in the function name. The `_rm` prefix for explicit FP rounding mode intrinsics ([Control of floating-point rounding mode](#control-of-frm)) is also omitted. The intrinsics under this scheme are the "overloaded intrinsics", which in parallel we have the "non-overloaded intrinsics" defined under [Explicit (Non-overloaded) naming scheme](#explicit-naming-scheme).
...
Since the main intent is to let the users put different value(s) of EEW and EMUL as input argument(s), the overloaded intrinsics do not omit the policy suffix. That is, the suffix listed under [Control of behavior of destination tail elements and destination inactive masked-off elements](#control-of-policy) is not omitted and is still encoded in the function name.
...
The naming scheme to prune everything except the instruction mnemonics is not available for all the intrinsics. Please see [Exceptions in the implicit (overloaded) naming scheme](#implicit-exception-naming) for overloaded intrinsics with irregular naming patterns.

Due to the limitations of the C language (without the aid of features like C++ templates), some intrinsics do not have an overloaded version. Therefore these intrinsics do not possess a simplified, EEW/EMUL-omitted interface. Please see [Un-supported intrinsics for implicit (overloaded) naming scheme](#unsupported-implicit-naming) for more detail.

### [](#implicit-exception-naming)Exceptions in the implicit (overloaded) naming scheme
...
#### [](#type-convert-instructions)Type-convert instructions
...
#### [](#vreinterpret-lmul-truncateextension-and-vsetvget)`vreinterpret`, LMUL truncate/extension, and `vset`/`vget`
...
### [](#unsupported-implicit-naming)Un-supported intrinsics for implicit (overloaded) naming scheme
...
These types are utilized through the segment load/store instruction intrinsics along with getters [vget](#pseudo-vget) and setters [vset](#pseudo-vset) to extract/combine them. The types listed in [Integer types](#integer-type) and [Floating-point types](#floating-point-type) all have tuple types. Types under the combination of `LMUL`, `NFIELD` follows the restriction by the RVV specification, `EMUL * NFIELDS ≤ 8`.
...
## [](#pseudo-intrinsics)Pseudo intrinsics
...
| |  As mentioned in [Control of number of elements to be processed](#control-of-vl), the vsetvlmax intrinsics do not necessarily map to the emission a vsetvli instruction of that exact SEW and LMUL provided. The actual value written to the vl control register is an implementation defined behavior and typically not known until runtime. |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |

### [](#pseudo-vreinterpret)`vreinterpret`
...
### [](#pseudo-vlmul%5Ftrunc)`vlmul_trunc`
...
### [](#agnostic-value-in-the-rvv-c-intrinsics)Agnostic value in the RVV C intrinsics
...
### [](#copying-vector-register-group-contents)Copying vector register group contents
...
### [](#the-passthrough-vd-argument-in-the-intrinsics)The passthrough (`vd`) argument in the intrinsics
...
### [](#assumption-of-vstart0-for-intrinsics-users)Assumption of `vstart=0` for intrinsics users.
...
### [](#assembly-generated-from-the-intrinsics)Assembly generated from the intrinsics
...
### [](#bookkeeping-of-configurations)Bookkeeping of configurations
...
### [](#strided-loadstore-with-stride-of-0)Strided load/store with stride of 0
...
### [](#leveraging-instructions-with-operand-mnemonics-of-vi)Leveraging instructions with operand mnemonics of `vi`
...
### [](#mixing-inline-assembly-and-intrinsics)Mixing inline assembly and intrinsics
...
### [](#the-new%5Fvl-argument-in-fault-only-first-load-intrinsics)The `new_vl` argument in fault-only-first load intrinsics
