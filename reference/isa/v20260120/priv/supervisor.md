# 11.1. Supervisor-Level ISA, Version 1.13

## [](#supervisor)11.1\. Supervisor-Level ISA, Version 1.13
...
### [](#11-1-1-supervisor-csrs)11.1.1\. Supervisor CSRs
...
#### [](#sstatus)11.1.1.1\. Supervisor Status (`sstatus`) Register
...
##### [](#11-1-1-1-1-base-isa-control-in-sstatus-register)11.1.1.1.1\. Base ISA Control in `sstatus` Register
...
##### [](#sum)11.1.1.1.2\. Memory Privilege in `sstatus` Register
...
##### [](#11-1-1-1-3-endianness-control-in-sstatus-register)11.1.1.1.3\. Endianness Control in `sstatus` Register
...
##### [](#11-1-1-1-4-previous-expected-landing-pad-elp-state-in-sstatus-register)11.1.1.1.4\. Previous Expected Landing Pad (ELP) State in `sstatus` Register
...
##### [](#supv-double-trap)11.1.1.1.5\. Double Trap Control in `sstatus` Register
...
#### [](#11-1-1-2-supervisor-trap-vector-base-address-stvec-register)11.1.1.2\. Supervisor Trap Vector Base Address (`stvec`) Register
...
#### [](#11-1-1-4-supervisor-timers-and-performance-counters)11.1.1.4\. Supervisor Timers and Performance Counters
...
#### [](#11-1-1-5-counter-enable-scounteren-register)11.1.1.5\. Counter-Enable (`scounteren`) Register
...
#### [](#11-1-1-6-supervisor-scratch-sscratch-register)11.1.1.6\. Supervisor Scratch (`sscratch`) Register
...
#### [](#11-1-1-7-supervisor-exception-program-counter-sepc-register)11.1.1.7\. Supervisor Exception Program Counter (`sepc`) Register
...
#### [](#scause)11.1.1.8\. Supervisor Cause (`scause`) Register
...
#### [](#11-1-1-9-supervisor-trap-value-stval-register)11.1.1.9\. Supervisor Trap Value (`stval`) Register
...
#### [](#sec:senvcfg)11.1.1.10\. Supervisor Environment Configuration (`senvcfg`) Register
...
#### [](#satp)11.1.1.11\. Supervisor Address Translation and Protection (`satp`) Register
...
When SXLEN=32, the only other valid setting for MODE is Sv32, a paged virtual-memory scheme described in [11.1.3\. Sv32: Page-Based 32-bit Virtual-Memory Systems](#sv32).

When SXLEN=64, three paged virtual-memory schemes are defined: Sv39, Sv48, and Sv57, described in [11.1.4\. Sv39: Page-Based 39-bit Virtual-Memory System](#sv39), [11.1.5\. Sv48: Page-Based 48-bit Virtual-Memory System](#sv48), and [11.1.6\. Sv57: Page-Based 57-bit Virtual-Memory System](#sv57), respectively. One additional scheme, Sv64, will be defined in a later version of this specification. The remaining MODE settings are reserved for future use and may define different interpretations of the other fields in `satp`.
...
| -------------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Value                | Name                       | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 01                   | BareSv32                   | No translation or protection.Page-based 32-bit virtual addressing (see [11.1.3\. Sv32: Page-Based 32-bit Virtual-Memory Systems](#sv32)).                                                                                                                                                                                                                                                                                                                                                               |
| **SXLEN=64**         |                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Value                | Name                       | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 01-789101112-1314-15 | Bare\-Sv39Sv48Sv57Sv64\-\- | No translation or protection. _Reserved for standard use_Page-based 39-bit virtual addressing (see [11.1.4\. Sv39: Page-Based 39-bit Virtual-Memory System](#sv39)).Page-based 48-bit virtual addressing (see [11.1.5\. Sv48: Page-Based 48-bit Virtual-Memory System](#sv48)).Page-based 57-bit virtual addressing (see [11.1.6\. Sv57: Page-Based 57-bit Virtual-Memory System](#sv57)). _Reserved for page-based 64-bit virtual addressing._ _Reserved for standard use_ _Designated for custom use_ |
...
#### [](#stimecmp)11.1.1.12\. Supervisor Timer (`stimecmp`) Register
...
### [](#11-1-2-supervisor-instructions)11.1.2\. Supervisor Instructions
...
* If _rs1_\=`x0` and _rs2_≠`x0`, the fence orders all reads and writes made to any level of the page tables, but only for the address space identified by integer register _rs2_. Accesses to _global_mappings (see [11.1.3.1\. Addressing and Memory Protection](#translation)) are not ordered. The fence also invalidates all address-translation cache entries matching the address space identified by integer register _rs2_, except for entries containing global mappings.
...
### [](#sv32)11.1.3\. Sv32: Page-Based 32-bit Virtual-Memory Systems

When Sv32 is written to the MODE field in the `satp` register (see[11.1.1.11\. Supervisor Address Translation and Protection (satp) Register](#satp)), the supervisor operates in a 32-bit paged virtual-memory system. In this mode, supervisor and user virtual addresses are translated into supervisor physical addresses by traversing a radix-tree page table. Sv32 is supported when SXLEN=32 and is designed to include mechanisms sufficient for supporting modern Unix-based operating systems.
...
#### [](#translation)11.1.3.1\. Addressing and Memory Protection
...
| |  An alternative PTE format would support different permissions for supervisor and user. We omitted this feature because it would be largely redundant with the SUM mechanism (see [11.1.1.1.2\. Memory Privilege in sstatus Register](#sum)) and would require more encoding space in the PTE. |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
#### [](#sv32algorithm)11.1.3.2\. Virtual Address Translation Process
...
Implementations may also execute the address-translation algorithm speculatively at any time, for any virtual address, as long as `satp` is active (as defined in [11.1.1.11\. Supervisor Address Translation and Protection (satp) Register](#satp)). Such speculative executions have the effect of pre-populating the address-translation cache.
...
### [](#sv39)11.1.4\. Sv39: Page-Based 39-bit Virtual-Memory System
...
#### [](#addressing-and-memory-protection)11.1.4.1\. Addressing and Memory Protection
...
The algorithm for virtual-to-physical address translation is the same as in [11.1.3.2\. Virtual Address Translation Process](#sv32algorithm), exceptLEVELS equals 3 andPTESIZE equals 8.

### [](#sv48)11.1.5\. Sv48: Page-Based 48-bit Virtual-Memory System
...
#### [](#addressing-and-memory-protection-1)11.1.5.1\. Addressing and Memory Protection
...
The algorithm for virtual-to-physical address translation is the same as in [11.1.3.2\. Virtual Address Translation Process](#sv32algorithm), exceptLEVELS equals 4 andPTESIZE equals 8.

### [](#sv57)11.1.6\. Sv57: Page-Based 57-bit Virtual-Memory System
...
#### [](#addressing-and-memory-protection-2)11.1.6.1\. Addressing and Memory Protection
...
The algorithm for virtual-to-physical address translation is the same as in [11.1.3.2\. Virtual Address Translation Process](#sv32algorithm), exceptLEVELS equals 5 andPTESIZE equals 8.

### [](#svnapot)11.1.7\. "Svnapot" Extension for NAPOT Translation Contiguity, Version 1.0
...
NAPOT PTEs behave identically to non-NAPOT PTEs within the address-translation algorithm in [11.1.3.2\. Virtual Address Translation Process](#sv32algorithm), except that:
...
### [](#svinval)11.1.9\. "Svinval" Extension for Fine-Grained Address-Translation Cache Invalidation, Version 1.0
...
### [](#sec:svadu)11.1.10\. "Svadu" Extension for Hardware Updating of A/D Bits, Version 1.0
...
### [](#sec:svrsw60t59b)11.1.12\. "Svrsw60t59b" Extension for PTE Reserved-for-Software Bits 60-59, Version 1.0
...
### [](#ssqosid)11.1.13\. "Ssqosid" Extension for Quality-of-Service (QoS) Identifiers, Version 1.0
...
#### [](#11-1-13-1-supervisor-resource-management-configuration-srmcfg-register)11.1.13.1\. Supervisor Resource Management Configuration (`srmcfg`) register
