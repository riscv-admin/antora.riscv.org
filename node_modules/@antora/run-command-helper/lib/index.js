...
function spawnCommand (cmdv, opts, stdio) {
  return new Promise((resolve, reject) => {
    try {
      const [cmd0, ...args] = cmdv
      const buffers = { stdout: [], stderr: [] }
      opts.stdio = toStdioOption(stdio, buffers)
      const p = spawn(cmd0, args, opts)
      p.on('close', (status, signal) => {
        if (!status) {
          const stdout = p.stdout ? encodeBufferArray(buffers.stdout, stdio.encoding) : null
          ...
          if (!stdout) return resolve(result)
        ...
        Object.assign(err, { exitStatus: status, spawnargs: p.spawnargs })
